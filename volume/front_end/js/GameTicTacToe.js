...
const NavigationManager = {
  navigate(page) {
	  ...
	  document.querySelector(".players-display_oelboukh").style.display = "none";
...
const OnlineGameManager = {
  ...
  async startOnlineGame(data) {
	  ...
	  const buttonsToEnable = ['play-friend', 'play-bot', 'show-history', 'play-online'];
		buttonsToEnable.forEach(id => {
			const button = document.getElementById(id);
			if (button) button.disabled = true;
		});

	  this.IsInsQueue = true;
	  try {
  ...
  async handleWebSocketOpen(game){
...
handleGameStart(data){
	  ...
	  document.getElementById("status").textContent = `It's your turn ` + GameState.playerRole;
  },
...
handleGameWinner(data) {
	...
	document.getElementById("status").textContent = 'No more turns';
},
...
const GameManager = {
  ...
  createBoard() {
		...
		if (GameState.gameMode == 'online')
			document.getElementById("status").textContent = "Opponent turn";
		else
  ...
  async sendOnlineMove(index, playerRole) {
	  try {
		  ...
		  if (game.status === 'tie') {
			  ...
			  GameState.gameOver = true;
			  GameManager.showWinModal('You win!');
			  const playerName = document.getElementById(GameState.playerRole === 'X' ? 'p1-name' : 'p2-name').textContent;
			  GameState.gameSocket.send(JSON.stringify({
				  type: 'game_winner',
  ...
  makeOfflineMove(index) {
	...
	if (this.checkForWinner()) {
		...
		const playerwins = winnerName.substring(0, 10);
		this.showWinModal(`${playerwins} wins!`);
		return;
	}
		...
		try {
			...
			games.forEach(game => {
				...
				if (game.winner === null || game.winner === undefined) {
					...
					result = winnerId === app.auth.currentUser.id ? 'win' : 'lost';
				}
				
				const resultText = result === 'win' ? 'Win' : result === 'lost' ? 'Lost' : 'Tie';
