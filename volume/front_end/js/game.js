...
function sendRequestWithoutWaiting(url, method, data = null, headers = {}) {
	const options = {
		method: method,
		headers: {
	...
	try {
		...
		console.log("Error Save In block")
	}
...
function initializeGame() {
	...
	function setAlltoZero() {
		...
		if (multiplayer) {
			...
			console.error(`Don't play games with me !!`);
		}
	}
...
function Score(ball, player1, player2) {
	...
	async function requestTournamentStart() {
		...
		try {
			...
			showError('Failed to start tournament. Please wait some time beffor trying again!');
			resetTournament();
			try{
			document.getElementById('tournamentModal').style.display = 'none';
			landingPage.style.display = 'flex';
			}catch(e){
				console.error('Keep going');
			}
	...
	function cancelTournament() {
		if (tournament.isActive || SettingUpTournament) {
			console.log("Cancelling tournament");
			sendRequestWithoutWaiting('/api/block/cancel/', 'POST', {});
			resetTournament();
			SettingUpTournament = false;
			
			// Hide tournament UI if visible
	...
	function resetTournament() {
		serverSideTournamentCleanup(false);
		sendRequestWithoutWaiting('/api/block/cancel/', 'POST', {});
		if (RequestController) {
			RequestController.abort();
			RequestController = null;
		...
		try {
			...
			console.error('it ok just keep going');
		}
		const setupElement = document.querySelector('.tournament-setup');
        const bracketElement = document.querySelector('.tournament-bracket');
        if (setupElement) setupElement.style.display = 'block';
        if (bracketElement) bracketElement.style.display = 'none';
        try {
			setAlltoZero();
        } catch (e) {
            console.log('Error resetting game state, continuing cleanup');
        }
	}

	window.resetTournament = resetTournament;

	function serverSideTournamentCleanup(useKeepalive = false) {
		try {
			if (window.tournament) {
				window.tournament.isActive = false;
			}
			...
			if (tournament && tournament.statusCheckInterval) {
				clearInterval(tournament.statusCheckInterval);
				tournament.statusCheckInterval = null;
			}
			...
			if (window.RequestController) {
				try {
					...
					console.log('Error aborting request controller:', e);
				}
				window.RequestController = null;
			}
			
			const cancelUrl = '/api/block/cancel/';
			const headers = {
				'Content-Type': 'application/json',
				'Authorization': `Bearer ${sessionStorage.getItem('accessToken')}`,
			};
			...
			if (useKeepalive) {
				fetch(cancelUrl, {
					...
					body: JSON.stringify({}),
					keepalive: true
				}).catch(e => console.log('Keepalive fetch error (expected during unload):', e));
				
				console.log('Tournament cancellation request sent with keepalive');
			} else {
				...
				fetch(cancelUrl, {
					...
					headers: headers,
					body: JSON.stringify({})
				}).catch(e => console.log('Normal fetch error:', e));
				
				console.log('Tournament cancellation request sent normally');
			}
		} catch (error) {
			console.error('Error during server-side tournament cleanup:', error);
			if (window.tournament) window.tournament.isActive = false;
			SettingUpTournament = false;
			isTournamentProcessing = false;
		}
	}
	...
	function forceTournamentEnd() {
		...
		.then(data => {
			// If tournament is active, force it to inactive on the client side
			if (data.is_active) {
				console.log("Force cleaning active tournament");
				tournament.isActive = false;
				SettingUpTournament = false;
			}
		})
		.catch(err => console.log("Error checking tournament status:", err));
	}
	...
	window.addEventListener('beforeunload', (e) => {
		...
		if (tournament && (tournament.isActive || SettingUpTournament)) {
			console.log("Cancelling tournament before page unload");
			forceTournamentEnd();
			...
			// Use a simple direct fetch with keepalive - make it as lightweight as possible
			const token = sessionStorage.getItem('accessToken');
			fetch('/api/block/cancel/', {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json',
			...
			try {
				serverSideTournamentCleanup(true);
			} catch (err) {
				console.error('Error in cleanup during unload:', err);
			}
			fetch('/api/block/cancel/', {
				method: 'POST',
				headers: {
